...
# Form implementation generated from reading ui file '361homework3.ui'
#
...
class Ui_root(object):
    ...
    def greedyMethod(self):
        ...
        a, b, c, d = Greedy.Greedy(Greedy.graph3, "s", "b")
        print(a)
        ttt = (b / int(self.KMforL.toPlainText())) * 2.18
        for i in Greedy.graph3:
            color = 'blue'
            for k in a:
                if i == k:
                    color = 'green'
            pyvis_graph.add_node(i, label=i, title=str(Greedy.heuristic[i]), color=color)

        for i in Greedy.graph3:
            for j in Greedy.graph3[i]:
                pyvis_graph.add_edge(i, j, label=Greedy.graph3[i].get(j), title=Greedy.graph3[i].get(j))
        self.path.setText(a)
    ...
    def aStarMethod(self):
        ...
        a, b, c, d = Astar.astar(Astar.g3, "s", "b")
        ttt = (b / int(self.KMforL.toPlainText())) * 2.18
        for i in Astar.g3:
            color = 'blue'
            for k in a:
                if i == k:
                    color = 'green'
            pyvis_graph.add_node(i, label=i, title=str(Astar.heuristic[i]), color=color)

        for i in Astar.g3:
            for j in Astar.g3[i]:
                pyvis_graph.add_edge(i, j, label=Astar.g3[i].get(j), title=Astar.g3[i].get(j))
